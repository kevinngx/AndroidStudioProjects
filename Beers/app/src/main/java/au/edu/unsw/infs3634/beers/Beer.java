...
public class Beer {
    ...
    public static final String FULL_URL = "https://sandbox-api.brewerydb.com/v2/beers/?key=0cafa2ad34d6df8c2e863a59c773394e&&withBreweries=Y";
    public static final String BASE_URL = "https://sandbox-api.brewerydb.com/v2/";
    public static final String KEY = "0cafa2ad34d6df8c2e863a59c773394e";

    @PrimaryKey
    @NonNull
    private String id;
    private String name;
    private String shortDescription = "TEST_SHORT_DESCRIPTION";
    ...
    class Style {
        ...
        public Style(String description, int ibuMin, int ibuMax, double abvMin, double abvMax, int srmMin, int srmMax) {
    ...
    @Ignore
    public Beer(String id, String name, String shortDescription, String description, double abv, int ibuMin, int ibuMax, int srmMin, int srmMax, String breweryName) {
        this.id = id;
        this.name = name;
...
//        this.srmMin = srmMin;
//        this.srmMax = srmMax;
        style = new Style(description, ibuMin, ibuMax, abv, abv, srmMin, srmMax);
        breweries.add(new Breweries(breweryName));
//        this.brewery = brewery;
    }


    public Beer(@NonNull String id, String name, String shortDescription) {
        this.id = id;
        this.name = name;
        this.shortDescription = shortDescription;
    }
...
//        this.srmMax = srmMax;
    ...
    public void setBrewery(String brewery) {
        String result;
        if (breweries.size() > 0) {
            this.breweries.get(0).name = brewery;
        } else {
            this.breweries.add(new Breweries(brewery));
        }
    }

    public static Beer getActualBeer(String id) {
        for(Beer beer : getActualBeers()) {
...
//                ", \nsrmMin=" + style.srmMin +
//                ", \nsrmMax=" + style.srmMax +
//                ", \nbrewery='" + breweries.get(0).name + '\'' +
                "\n}";
    }

    public static ArrayList<Beer> getActualBeers() {
        ArrayList<Beer> beerList = new ArrayList<>();
        beerList.add(JsonConverter.jsonToBeer(GetRawData.getTestJson()));
        return beerList;
    }
    ...
    public static ArrayList<Beer> getDummyBeers() {
        ...
        beers.add(new Beer("CwbQiB", "Grand Royal", "Imperial or Double India Pale Ale", "Packed to the brim with Galaxy, 007, and Mosaic hops this Double IPA is big, juicy, and completely satisfying. Notes of tropical fruit and pine are supported by a clean malt backbone.", 8.3, 65, 100, 5, 13, "Zero Gravity Craft Breweries"));
